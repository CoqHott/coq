...
let typecheck_inductive env mie =
  ...
  let inds =
    Array.map (fun ((id,full_arity,sign,template,def_level,inf_level),cn,lc,(is_unit,clev))  ->
      let infu = 
	...
	in
      ...
      let arity =
        ...
        | Polymorphic_ind_entry _ | Cumulative_ind_entry _ ->
          if template then anomaly ~label:"polymorphic_template_ind" Pp.(str "Template polymorphism and full polymorphism are incompatible.");
          full_polymorphic ()
      in
	(id,cn,lc,(sign,arity)))
    inds
