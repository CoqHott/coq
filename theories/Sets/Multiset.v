...
Polymorphic Cumulative Inductive multiset (A : Type) : Type :=
  Bag : (A -> nat) -> multiset A.
...
Section multiset_defs.
  ...
  Definition multiplicity (m:multiset A) (a:A) : nat := let (f) := m in f a.

  (** multiset equality *)
  Definition meq (m1 m2:multiset A) :=
    forall a:A, multiplicity m1 a = multiplicity m2 a.

  Lemma meq_refl : forall x:multiset A, meq x x.
  Proof.
    destruct x; unfold meq; reflexivity.
  Qed.

  Lemma meq_trans : forall x y z:multiset A, meq x y -> meq y z -> meq x z.
  Proof.
    unfold meq.
  ...
  Lemma meq_sym : forall x y:multiset A, meq x y -> meq y x.
  Proof.
    unfold meq.
  ...
  (** multiset union *)
  Definition munion (m1 m2:multiset A) :=
    Bag (fun a:A => multiplicity m1 a + multiplicity m2 a).

  Lemma munion_empty_left : forall x:multiset A, meq x (munion EmptyBag x).
  Proof.
    unfold meq; unfold munion; simpl; auto.
  Qed.

  Lemma munion_empty_right : forall x:multiset A, meq x (munion x EmptyBag).
  Proof.
    unfold meq; unfold munion; simpl; auto.
  Qed.

  Lemma munion_comm : forall x y:multiset A, meq (munion x y) (munion y x).
  Proof.
  ...
  Lemma munion_ass :
    forall x y z:multiset A, meq (munion (munion x y) z) (munion x (munion y z)).
  Proof.
  ...
  Lemma meq_left :
    forall x y z:multiset A, meq x y -> meq (munion x z) (munion y z).
  Proof.
    unfold meq; unfold munion; unfold multiplicity.
    destruct x; destruct y; destruct z.
  ...
  Lemma meq_right :
    forall x y z:multiset A, meq x y -> meq (munion z x) (munion z y).
  Proof.
    unfold meq; unfold munion; unfold multiplicity.
    destruct x; destruct y; destruct z.
    intros; elim H; auto.
  ...
  Lemma munion_rotate :
    forall x y z:multiset A, meq (munion x (munion y z)) (munion z (munion x y)).
  Proof.
    intros; apply (op_rotate (@multiset _) munion meq).
      apply munion_comm.
      apply munion_ass.
      exact meq_trans.
      exact meq_sym.
  ...
  Lemma meq_congr :
    forall x y z t:multiset A, meq x y -> meq z t -> meq (munion x z) (munion y t).
  Proof.
    intros; apply (cong_congr (@multiset _) munion meq); auto using meq_left, meq_right.
      exact meq_trans.
  Qed.

  Lemma munion_perm_left :
    forall x y z:multiset A, meq (munion x (munion y z)) (munion y (munion x z)).
  Proof.
    intros; apply (perm_left (@multiset _) munion meq); auto using munion_comm, munion_ass, meq_left, meq_right, meq_sym.
      exact meq_trans.
  Qed.

  Lemma multiset_twist1 :
    forall x y z t:multiset A,
      meq (munion x (munion (munion y z) t)) (munion (munion y (munion x t)) z).
  Proof.
    intros; apply (twist (@multiset _) munion meq); auto using munion_comm, munion_ass, meq_sym, meq_left, meq_right.
    exact meq_trans.
  Qed.
