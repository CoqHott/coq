...
Polymorphic Cumulative Inductive memo_val (A: nat -> Type) (f: forall n, A n) : Type :=
  memo_mval: forall n, A n -> memo_val A f.

Arguments memo_mval {_ _ } _ _.
...
Definition memo_get_val n (v: memo_val A f): A n :=
match v with
| memo_mval m x =>
...
rewrite (memo_get_correct (@memo_val _ _) mf n); simpl.
...
Let mg v :=  match v : memo_val A f with
             memo_mval n1 v1 => memo_mval (f:=f) (S n1) (g n1 v1) end.
...
rewrite (imemo_get_correct (@memo_val _ _) mf mg); simpl.
- case (is_eq n n); simpl; auto; intros e.
  assert (e = eq_refl n).
  + apply eq_proofs_unicity.
    induction x as [| x Hx]; destruct y as [| y].
