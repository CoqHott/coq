...
let cl_typ_ord t1 t2 = match t1, t2 with
  ...
  | CL_PROJ c1, CL_PROJ c2 -> Projection.Repr.compare c1 c2
...
let string_of_class = function
  ...
  | CL_PROJ sp ->
    string_of_qualid (shortest_qualid_of_global Id.Set.empty
                        (ConstRef (Projection.Repr.constant sp)))
  | CL_IND sp ->
...
let subst_coercion (subst, c) =
  ...
  let clp = Option.smartmap (subst_proj_repr subst) c.coercion_is_proj in
...
let declare_coercion coef ?(local = false) ~isid ~src:cls ~target:clt ~params:ps =
  let isproj = 
    match coef with
    | ConstRef c -> Environ.lookup_projection_constant c (Global.env ())
    | _ -> None
  in
  let c = {
    coercion_type = coef;
